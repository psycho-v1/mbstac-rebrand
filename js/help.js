...
function tile(title, value, note, helpKey) {
  ...
  return `<div class="tile"><button type="button" class="info" onclick="this.nextElementSibling.classList.toggle('on')" aria-label="Explain">i</button><strong>${title}</strong><div class="value">${value}</div><div class="helpbox">${text}</div><div class="hint">${note || ""}</div></div>`;
}
